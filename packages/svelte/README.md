...
The `border` prop (component and action) passes straight through to
`@cornerkit/core` (v1.2+ API), so every border style works: solid, dashed,
dotted, custom dash patterns, and gradients.
...
<!-- Works with the action too -->
<div use:squircle={{ radius: 16, border: { width: 2, style: 'dotted', color: '#10b981' } }}>
  Dotted
</div>
```
